import { Mic2, LogOut, Database } from "lucide-react";
import { useAuth } from "../contexts/AuthContext";
import { useNavigation } from "../contexts/NavigationContext";
import EditableText from "./EditableText";

export default function AppHeader() {
  const { user, signOut } = useAuth();
  const { activePage, navigate } = useNavigation();

  return (
    <header className="bg-white/90 backdrop-blur-md border-b border-stone-200/80 sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3" dir="rtl">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center shadow-sm flex-shrink-0">
            <Mic2 size={20} className="text-white" strokeWidth={1.8} />
          </div>
          <div>
            <EditableText
              contentKey="header_title"
              defaultValue="יידיש טרענסילעישן - איבערטייטשער"
              as="h1"
              className="text-lg font-bold text-stone-900 leading-tight font-hebrew"
              dir="rtl"
            />
            <p className="text-stone-400 text-[11px] font-hebrew" dir="ltr">
              ivrit-ai / yi-whisper-large-v3-turbo
            </p>
          </div>
        </div>

        <nav className="flex items-center bg-stone-100 rounded-xl p-1 gap-1">
          <button
            onClick={() => navigate("transcription")}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-150 ${
              activePage === "transcription"
                ? "bg-white text-stone-900 shadow-sm"
                : "text-stone-500 hover:text-stone-700"
            }`}
          >
            <Mic2 size={15} />
            <span className="hidden sm:inline">Transcribe</span>
          </button>
          <button
            onClick={() => navigate("dataset")}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-150 ${
              activePage === "dataset"
                ? "bg-white text-stone-900 shadow-sm"
                : "text-stone-500 hover:text-stone-700"
            }`}
          >
            <Database size={15} />
            <span className="hidden sm:inline">Dataset</span>
          </button>
        </nav>

        <div className="flex items-center gap-3">
          <span
            className="text-stone-400 text-[0.8rem] font-hebrew hidden md:block truncate max-w-[180px]"
            dir="ltr"
          >
            {user?.email}
          </span>
          <button
            onClick={signOut}
            className="flex items-center gap-1.5 text-stone-500 hover:text-red-600 hover:bg-red-50 px-3 py-2 rounded-lg transition-colors duration-150 text-sm font-medium"
            dir="rtl"
            title="ארויסלאגן"
          >
            <LogOut size={15} />
            <EditableText
              contentKey="header_logout"
              defaultValue="אַרויסלאָגן"
              as="span"
              className="hidden sm:inline font-hebrew"
              dir="rtl"
            />
          </button>
        </div>
      </div>
    </header>
  );
}
